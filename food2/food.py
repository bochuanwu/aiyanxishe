...
#train_df[train_df[1]>1]=1
train_df[0] = train_df[0].apply(lambda x: './train/{0}'.format(x) + '.jpg')
#train_df2 = pd.read_csv('./train2.csv',header=None)
#train_df2[0] = train_df2[0].apply(lambda x: './test/{0}'.format(x) + '.jpg')
#print(train_df)
...
class VisitNet(nn.Module):
    def __init__(self):
        ...
        self.resnet = model
        #num_ftrs = model.aux_logits.fc.in_features
        #model.aux_logits.fc = nn.Linear(num_ftrs, 4)
        #self.resnet = model
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        print(top1.avg.data.cpu().numpy())
        print(losses.avg)
        return top1.avg.data.cpu().numpy()
        #return losses.avg
...
for flod_idx, (train_idx, val_idx) in enumerate(skf.split(train_df[0].values, train_df[0].values)):
    ...
    train_loader = torch.utils.data.DataLoader(
        QRDataset(train_df.iloc[train_idx],
                  transforms.Compose([
                      ...
                      transforms.RandomChoice([transforms.Resize([400, 400]), transforms.CenterCrop([400, 400])]),
    ...
    #criterion = CrossEntropyLabelSmooth(4)
    ...
    for epoch in range(15):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), './0_wide_fold{0}.pt'.format(flod_idx))
    model.load_state_dict(torch.load('./0_wide_fold{0}.pt'.format(flod_idx)))
    if final_acc == 0:
        final_acc = validate(val_loader, model, criterion)
    else:
        final_acc+= validate(val_loader, model, criterion)
print(final_acc/5)
